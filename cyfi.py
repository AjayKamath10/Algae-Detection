...
def save_metadata_and_predictions(df, predictions, keep_metadata):
    if keep_metadata:
        ...
        pd.DataFrame(predictions, columns=['Estimated Cyanobacteria Density']).to_csv(
...
def cyfi():
    parser = argparse.ArgumentParser(
        description='Predict cyanobacteria density from satellite images.')
    parser.add_argument(
        'file', type=str, help='Path to the CSV file containing latitude, longitude, and date.')
    parser.add_argument('--keep-metadata', action='store_true',
                        help='Save metadata and predictions to separate CSV files.')
